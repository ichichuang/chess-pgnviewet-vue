...
6. 大屏操作员选择赛事与组别；页面按确认规则解析轮次，并在一个视口中自适应同时展示该轮全部对阵及真实故障状态。
...
教师赛事讲解默认展开最新已完成轮次；没有已完成轮次时使用当前进行中轮次；否则使用最近的待开始轮次或首个有效轮次。场外大屏有独立规则：有效显式 URL 轮次优先；自动解析时检查来源顺序中的最后一个有效轮次，最后轮进行中则使用该轮，最后轮未开始则使用其前最近已完成轮，最后轮已完成则使用该轮，最后轮生命周期未知时只使用最新确认已完成轮，否则显示稳定的无已完成轮/生命周期未知状态。大屏不得仅因轮次最新而选择未开始轮，两类场景不得共用通用默认规则。
...
大屏是独立、只读、远距离观看表面，复用统一棋盘组件和 Token，不复用教学右侧面板。入口页选择赛事与组别；展示页自动解析轮次，只有有效显式 display URL 轮次才作为操作员覆盖。自动解析遵循第 17 节的末轮真值规则。所选轮次全部对阵保持来源顺序并在一个视口中同时可见，不分页、不轮播、不聚焦覆盖、不隐藏对阵溢出，也不产生页面滚动。

每格上部使用“左方选手 / 居中 VS / 右方选手”三列头部，保留真实台号、双方姓名、等级分/队伍元数据（仅来源存在时）及结果/状态；下部保留正方棋盘舞台。时钟只有在来源合同确认后显示。断线或陈旧棋盘在存在最后可信真实局面时保留该局面并明确标记；不得生成起始局面或其他成功回退。

## 22. 智能大屏布局算法

批准策略：**一个视口内覆盖全部对阵的确定性自适应网格**。对列数 `c = 1..boardCount` 的每个候选计算 `r = ceil(boardCount/c)`：

```text
usableWidth      = stageWidth - (c-1)*cellGap
usableHeight     = stageHeight - (r-1)*cellGap
...
unusedCells      = r*c - boardCount
aspectDistortion = abs(tileWidth-boardCapacity) / max(tileWidth,boardCapacity,1)
```

选择 `boardSize` 最大的候选；在浮点容差内相同时，先选空格更少的候选，再选 `aspectDistortion` 更低的候选，最后以更少列作为确定性收束。网格使用相同行高、列宽和棋盘尺寸，始终保留来源对阵顺序；一盘或少量对阵自然获得更大棋盘，数量增加时各格逐步缩小。不得用分页、隐藏溢出或聚焦覆盖代替全部同屏。

格间距、选手头部高度、文本最小区域和状态覆盖几何由全局 Token 提供。`OD-05` 的最终最小/首选棋盘尺寸及 `OD-06` 的最终边距/间距仍为开放决定；本算法的当前 Token 只是本切片的可审查临时实现，不关闭这些决定。

```mermaid
flowchart TD
  A[读取舞台尺寸/对阵数/头部与间距] --> C[枚举 1..N 列]
  C --> S[计算行数/格宽/格高/正方棋盘]
  S --> B{还有候选?}
  B -->|是| C
  B -->|否| K[最大棋盘；同分少空格再低失真]
  K --> G[按来源顺序渲染全部同屏对阵]
```
...
动效只用于解释状态变化：棋子移动、面板展开和来源交接。遵循项目 GSAP 权威，组件卸载时清理。`prefers-reduced-motion` 下取消弹性、路径和自动平移动效，棋子与页面直接切换；场外大屏不轮播或换页。实时状态不可仅靠闪烁表达。
...
**SHOULD**：最近教学材料、棋局信息与标签、教学笔记、候选线导入、实时新鲜度、完成后导入分析、大屏全对阵自适应同屏、用户主题/布局偏好。

**COULD**：棋盘图片、打印/教学报告、音效、课次级笔记、组织棋库。
