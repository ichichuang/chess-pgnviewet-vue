...
| 大屏原则     | 按实测舞台最大化正方棋盘，并让所选轮次全部对阵同时可见              |
...
6. 允许模块内部换行、列表化或分页。
7. 场外大屏继续枚举全部列数候选并等比例缩小同屏格子，不删除、隐藏或分页任何对阵。
...
- CURRENT_IMPLEMENTED：公开赛事摘要、显式组别/轮次选择、修订的自动末轮规则，以及由实测舞台驱动的全对阵自适应同屏；每台没有真实 FEN 时保持相同正方形合同阻断舞台。
- APPROVED_TARGET：合同就绪后在相同全对阵格子中显示规范只读棋盘、最后可信局面和有真实证据的每台故障状态。
...
| Profile    | 设计策略                                             | 不允许的推断                       |
| ---------- | ---------------------------------------------------- | ---------------------------------- |
| 1080p 16:9 | 枚举全部列数候选；全对阵同屏；一盘/少量显著放大      | 不分页、隐藏对阵或固化最终可读阈值 |
| 4K 16:9    | 按实测 CSS 舞台重新枚举并保持全部同屏                | 不按物理像素或固定模板猜测列数     |
| 21:9       | 宽度自然影响候选结果；正方棋盘、选手头部和状态均保留 | 不横向拉伸棋盘，不删除状态区       |

这些 profile 不取代算法。每次布局均根据实测舞台、必需选手头部高度、间距和对阵数计算。

### 8.3 多棋盘算法边界

    boardSize = min(tileWidth, tileHeight - playerHeaderHeight)

- 枚举列数 `1..pairingCount`，行数为 `ceil(pairingCount / columns)`。
- 先最大化真实正方棋盘尺寸；同分先减少空格，再降低格子宽高失真，最后以更少列确定性收束。
- 全部对阵保持来源顺序、等列宽、等行高和相同棋盘尺寸；状态变化不折叠格子。
- 不分页、不轮播、不聚焦覆盖、不隐藏溢出，不用额外赛事摘要格填补空位。
- OD-05 未关闭前，最终 minimum/preferred board size 仍由真实设备验收决定。
- OD-06 未关闭前，最终 safe margin 与 cell gap 仍为开放决定；本页不实现 rotation。
...
- prefers-reduced-motion 下 drawer/sheet 直接到最终状态；大屏没有自动平移或翻页。
...
| 场外大屏    | 赛事上下文、台号/棋手/结果状态、全部对阵同屏与真实数据状态 |
| 登录        | 安全返回、账号、密码、提交、错误                           |
...
| OD    | 响应式影响            | 固定边界                   |
| ----- | --------------------- | -------------------------- |
| OD-05 | 大屏最小/首选棋盘尺寸 | 全部对阵同屏且棋盘正方形   |
| OD-06 | 大屏最终边距、间距    | 当前 Token 仅为临时实施值  |
