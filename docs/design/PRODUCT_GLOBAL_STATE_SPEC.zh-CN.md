...
- 状态触发与焦点：[产品全局交互规范](./PRODUCT_GLOBAL_INTERACTION_SPEC.zh-CN.md)
- 状态覆盖层：[共用覆盖层与对话框规范](./PRODUCT_COMMON_OVERLAYS_AND_DIALOGS_SPEC.zh-CN.md)
- 状态几何与滚动：[产品全局布局规范](./PRODUCT_GLOBAL_LAYOUT_SPEC.zh-CN.md)
- 状态视觉角色：[产品 UI 设计系统](./PRODUCT_UI_DESIGN_SYSTEM.zh-CN.md)
- 状态实现责任：[产品组件责任规范](./PRODUCT_COMPONENT_RESPONSIBILITY_SPEC.zh-CN.md)
...
| `OD-05` | 大屏必须保持全对阵同屏与正方棋盘     | 最小/首选棋盘尺寸               |
| `OD-06` | 本页禁止轮播；当前间距仅为临时值     | 最终边距与间距                  |
...
6. 非模态状态不抢焦点，模态状态初焦点与返回焦点符合[产品全局交互规范](./PRODUCT_GLOBAL_INTERACTION_SPEC.zh-CN.md)；
7. 状态图标不是唯一信息载体，播报不泄漏技术或敏感信息；
8. 所有当前实现差异均引用[实施纠正清单](./PRODUCT_IMPLEMENTATION_CORRECTION_BACKLOG.zh-CN.md)，不把目标行为伪装成当前事实。
