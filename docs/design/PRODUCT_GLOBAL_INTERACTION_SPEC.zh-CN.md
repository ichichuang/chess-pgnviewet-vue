...
- 场外大屏：操作员控制 → 当前页棋盘卡片 → 分页/聚焦退出 → 状态。纯观众模式不出现不可用的操作员控件。
...
- 动效只解释来源交接、面板展开、棋子移动、结果到达、抽屉和大屏换页；不得延迟状态提交或焦点移动。
...
- 大屏自动轮播如果未来在 `OD-06` 关闭后实现，必须可暂停；减弱动效下默认不自动换页。当前不得把建议停留时间写成运行时常量。
...
| `OD-05` | 大屏低于确认可读阈值时分页           | 最小与首选棋盘尺寸                         |
| `OD-06` | 翻页可手动控制、自动行为可暂停       | 边距、间距、是否/何时自动轮播及停留时间    |
