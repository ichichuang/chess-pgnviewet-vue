...
实施以[产品 UI 设计索引](./PRODUCT_UI_DESIGN_INDEX.zh-CN.md)为入口，并同时服从以下唯一责任文档：

- [产品 UI 设计系统](./PRODUCT_UI_DESIGN_SYSTEM.zh-CN.md)：现有 Token、控件和视觉状态。
- [全局布局规范](./PRODUCT_GLOBAL_LAYOUT_SPEC.zh-CN.md)：壳、区域、几何和滚动。
- [全局交互规范](./PRODUCT_GLOBAL_INTERACTION_SPEC.zh-CN.md)：动作、输入、焦点和恢复。
- [全局状态规范](./PRODUCT_GLOBAL_STATE_SPEC.zh-CN.md)：状态标题、解释、动作和恢复。
- [响应式规范](./PRODUCT_RESPONSIVE_SPEC.zh-CN.md)：桌面、平板、窄屏和场外大屏组合。
- [组件责任规范](./PRODUCT_COMPONENT_RESPONSIBILITY_SPEC.zh-CN.md)：现有组件、概念责任和依赖边界。
- [Naive UI 映射](./PRODUCT_NAIVE_UI_MAPPING.zh-CN.md)：基础控件候选与项目适配器边界。
- [公共覆盖层与对话框规范](./PRODUCT_COMMON_OVERLAYS_AND_DIALOGS_SPEC.zh-CN.md)：模态、抽屉、sheet 与返回焦点。
- [实施纠正清单](./PRODUCT_IMPLEMENTATION_CORRECTION_BACKLOG.zh-CN.md)：28 项当前实现差异及关闭证据。
...
| C9   | Venue display composition                          | C2、C3、C6、C8                | 独立只读壳、修订轮次规则与全对阵自适应同屏          | Venue display owner        |
...
| ---- | ---------------------------------------------------------------------------- | ----------------- | ----------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------- |
| 1    | [登录](./pages/LOGIN_PAGE_SPEC.zh-CN.md)                                     | C1、C3、C4        | `COR-009/011/013/021`                                 | 仅使用现有账户合同和安全 return                   | 登录默认、校验、请求、错误凭据、过期、无权限/不可用与安全返回 |
| 2    | [赛事列表](./pages/COMPETITION_LIST_PAGE_SPEC.zh-CN.md)                      | C1–C4、C8         | `COR-010/012/015/022`                                 | 仅现有公开读取合同                                | URL 筛选/分页、加载、保留刷新、空、失败和响应式浏览           |
| 3    | [赛事详情](./pages/COMPETITION_DETAIL_PAGE_SPEC.zh-CN.md)                    | C1–C4、C8         | `COR-005/006/010/023`                                 | 团体聚合和权威回放保持阻断                        | 真实个人详情、层级选择、局部状态和真实动作可用性              |
| 4    | [统一工作区](./pages/UNIFIED_WORKSPACE_PAGE_SPEC.zh-CN.md)                   | C1–C7             | `COR-001/002/003/008/014/016/017/018/019/020/021/025` | `OD-01/02/03/04/07/08/09/10/11`；远端合同保持阻断 | 本地 PGN/手工局面、模式门控、安全只读壳和已确认状态           |
| 5    | [兼容与不可用](./pages/COMPATIBILITY_AND_UNAVAILABLE_SURFACES_SPEC.zh-CN.md) | C2–C5、C8         | `COR-006/007/008/009/010/018/021`                     | cloud/share/replay/live 合同未闭合                | 安全 handoff 或真实不可用；绝不渲染第二工作区                 |
| 6    | [设置](./pages/SETTINGS_SURFACE_SPEC.zh-CN.md)                               | C1、C3、C4        | `COR-011/012/013/026/027`                             | `OD-03/04/11` 保持开放                            | 已实现主题与布局状态；目标设置按分类显示且不伪持久化          |
| 7    | [场外大屏](./pages/VENUE_DISPLAY_PAGE_SPEC.zh-CN.md)                         | C1–C4、C6、C8、C9 | `COR-004/005/007/012/014/015/024`                     | `OD-05/06/07/08`；实时成功合同保持阻断            | 当前公开对阵 + 独立只读布局 + 实时不可用/生命周期壳           |
...
- 场外大屏候选布局评分的内部算法，只要仍满足全对阵同屏、稳定来源顺序、正方等几何和 `OD-05/06` 边界。
