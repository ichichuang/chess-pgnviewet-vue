> **非权威性阅读镜像提示**
> 本文档是原始文件 `docs/design/PRODUCT_GLOBAL_LAYOUT_SPEC.zh-CN.md` 的中文阅读镜像，仅供人工浏览。产品、设计、架构与实现权威仍保留在原始源路径，请勿将本镜像作为实施或自动化编辑的依据。
...
- 总索引：[产品逐页 UI 设计索引](UI设计索引.md)
- 视觉：[产品 UI 设计系统](UI设计系统.md)
- 断点与页面折叠：[响应式规范](响应式规范.md)
- 交互与焦点：[全局交互规范](全局交互规范.md)
- 状态占位：[全局状态规范](全局状态规范.md)
- 覆盖层：[共用覆盖层与对话框](通用浮层与对话框规范.md)
- 组件所有权：[组件责任规范](组件职责规范.md)
- 当前差异：[实施纠正清单](实现修正待办清单.md)

上游产品结构见[产品信息架构](../产品/产品信息架构.json)和[产品设计蓝图](../产品/产品设计蓝图.md)。
...
页面规范：[兼容入口与不可用表面](页面/兼容性与不可用界面规范.md)。
...
行为和视觉分别引用[共用覆盖层与对话框](通用浮层与对话框规范.md)和[产品 UI 设计系统](UI设计系统.md)。
